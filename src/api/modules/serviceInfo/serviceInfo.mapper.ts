...
export type MapperResponse =
  | Result<GetServiceInfoDto, DomainError>
  | Result<unknown, DomainError>

@Injectable()
export class GetServiceInfoMapper {
  toResponse(
    result: Result<GetServiceInfoOutput, DomainError>
  ): MapperResponse {
    ...
    if (errors.length) {
      return Result.fail(
        serviceInfoErrors.mapperValidation({
          origin: 'GetServiceInfoMapper.toResponse',
          details: this.toErrorDetails(errors)
        })
      )
    }
